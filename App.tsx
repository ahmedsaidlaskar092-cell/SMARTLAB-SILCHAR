...
import type { User, Test, Booking, Report, SampleStatus, Notification, PaymentStatus } from './types';
...
const App: React.FC = () => {
  ...
  const updateBookingDetails = useCallback((bookingId: string, updatedDetails: Partial<Booking>) => {
    ...
    if (updatedDetails.paymentStatus === 'Fully Paid') {
        const booking = bookings.find(b => b.id === bookingId);
        if (booking) {
            sendNotification({
                userId: booking.userId,
                title: "Payment Completed",
                message: `Your payment of $${booking.totalAmount - booking.discount} for Booking #${booking.id.slice(0,6)} is fully completed. Thank you.`
            });
        }
    }
  }, [bookings]);
  ...
  const appContextValue: IAppContext = {
    ...
    updateUser, deleteUser, addTest, updateTest,
  };
